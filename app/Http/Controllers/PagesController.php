...
class PagesController extends Controller
{
   ...
   public function rechercherCommandes(Request $request)
    ...
    {
        // Validate the request data to ensure dates are present and properly formatted
        $request->validate([
            'date_depart' => 'required|date',
            'date_fin' => 'required|date|after_or_equal:date_depart',
        ]);
    
        // Retrieve the date inputs
        $d = $request->input('date_depart');
        $f = $request->input('date_fin');
    
        // Execute the query with non-empty date values
        $achats = DB::table('achats')
                    ->whereBetween('created_at', [$d, $f])
                    ->get();
    
        return view('pages.rechercherAchats')->with('achats', $achats);
    }
